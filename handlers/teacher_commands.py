    ...
    for ts in teacher_subjects:
        ...
        for event in events:
            ...
            if subject_name.lower() in event_title.lower():
                all_events.append(
                    {"event": event, "group": group_name, "subject": subject_name}
                )

    if not all_events:
        await message.answer(
            "📭 У вас немає запланованих занять", parse_mode="Markdown"
        )
        return
...
async def my_subjects_command(message: Message):
    ...
    if user["role"] != UserRole.TEACHER.value:
        await message.answer(
            "❌ Ця команда доступна лише викладачам", parse_mode="Markdown"
        )
        return
    ...
    if not teacher_subjects:
        await message.answer(
            "📭 У вас немає призначених предметів", parse_mode="Markdown"
        )
        return
...
async def edit_lesson_command(message: Message, state: FSMContext):
    ...
    if not event:
        await message.answer(
            f"❌ Заняття з ID `{event_id}` не знайдено", parse_mode="Markdown"
        )
        return
...
async def process_new_time(message: Message, state: FSMContext):
    ...
    if time_input != "-" and not validate_time(time_input):
        await message.answer(
            "❌ Неправильний формат часу! Використовуйте HH:MM", parse_mode="Markdown"
        )
        return
...
async def process_new_title(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введіть нову аудиторію (або `-` щоб залишити без змін):", parse_mode="Markdown"
    )
...
async def process_new_room(message: Message, state: FSMContext):
    ...
    final_date = (
        new_date
        if new_date
        else (event["date"] if isinstance(event, dict) else event[1])
    )
    final_time = (
        new_time
        if new_time
        else (event["time"] if isinstance(event, dict) else event[2])
    )
    final_title = (
        new_title
        if new_title
        else (event["title"] if isinstance(event, dict) else event[3])
    )
    final_room = (
        room_input
        if room_input != "-"
        else (event["room"] if isinstance(event, dict) else event[4])
    )
...
async def delete_lesson_command(message: Message):
    ...
    try:
        ...
        await message.answer(
            "❌ Невірний формат ID! ID повинно бути числом", parse_mode="Markdown"
        )
        return
    ...
    if success:
        await message.answer(
            f"✅ Заняття з ID `{event_id}` видалено", parse_mode="Markdown"
        )

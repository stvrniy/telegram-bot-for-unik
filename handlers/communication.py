...
async def messages_command(message: Message):
    ...
    if not messages_list:
        await message.answer(
            header + "📭 У вас немає повідомлень", parse_mode="Markdown"
        )
        return
...
async def unread_command(message: Message):
    ...
    if unread_count == 0:
        await message.answer(
            "✅ У вас немає непрочитаних повідомлень", parse_mode="Markdown"
        )
        return
...
async def msg_command(message: Message):
    ...
    if not recipients:
        await message.answer(
            f"❌ Користувача '{recipient_name}' не знайдено", parse_mode="Markdown"
        )
        return

    if len(recipients) > 1:
        # Multiple matches - ask for clarification
        response = f"❌ Знайдено кілька користувачів з ім'ям '{recipient_name}':\n\n"
    ...
    try:
        await message.bot.send_message(
            recipient_id, message_text, parse_mode="Markdown"
        )
    except Exception as e:
        await message.answer(
            f"❌ Не вдалося відправити повідомлення: {e}", parse_mode="Markdown"
        )
        return
    ...
    await message.answer(
        f"✅ Повідомлення відправлено {sender_name}!", parse_mode="Markdown"
    )
...
async def contact_group_leader_command(message: Message):
    ...
    leader_name = (
        group_leader["full_name"] if isinstance(group_leader, dict) else group_leader[2]
    )
...
async def process_recipient_selection(message: Message, state: FSMContext):
    ...
    leader_id = (
        group_leader["user_id"] if isinstance(group_leader, dict) else group_leader[0]
    )
...
async def process_message_to_recipient(message: Message, state: FSMContext):
    ...
    # Send message
    message_text = f"📩 *Повідомлення від {sender_name}:*\n\n{message.text}"
    ...
    if group_name:
        ...
        if group_subjects:
            ...
            for subj in group_subjects:
                ...
                teacher_name = (
                    subj["teacher_name"] if isinstance(subj, dict) else subj[2]
                )
                if teacher_name:
    ...
    if not subjects:
        ...
        default_subjects = [
            (
                ...
                "ВМ",
                4,
                "Курс вищої математики для технічних спеціальностей",
            ),
            ...
            (
                ...
                "АСД",
                6,
                "Алгоритми та основні структури даних",
            ),

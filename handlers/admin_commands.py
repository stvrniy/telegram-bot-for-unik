...
async def process_group_message(message: Message, state: FSMContext):
    ...
    for user in users:
        try:
            ...
            logger.error(
                f"Не вдалося відправити повідомлення користувачу {user[0]}: {e}"
            )

    await message.answer(
        f"✅ Повідомлення відправлено {sent_count} студентам групи {group_name}"
    )
    await state.clear()
...
async def notify_student_command(message: Message):
    ...
    for student in students:
        try:
            ...
            logger.error(
                f"Не вдалося надіслати повідомлення студенту {student[0]}: {e}"
            )

...
async def process_ics_file(message: Message, state: FSMContext):
    ...
    if not message.document:
        await message.answer(
            "❌ Будь ласка, надішліть файл з розкладом",
            parse_mode="Markdown"
        )
        return
    ...
    if not document.file_name.endswith('.ics'):
        await message.answer(
            "❌ Файл повинен мати розширення .ics",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        if not events:
            await message.answer(
                "❌ Не вдалося розпізнати події у файлі",
                parse_mode="Markdown"
            )
            await state.clear()
            return
...
async def week_schedule_command(message: Message):
    ...
    if not events:
        await message.answer(
            f"📭 Для *{group_name}* немає запланованих занять",
            parse_mode="Markdown"
        )
        return
    ...
    if not week_events:
        await message.answer(
            f"📭 На цьому тижні для *{group_name}* немає занять",
            parse_mode="Markdown"
        )
        return

...
def test_get_events_function(simple_db):
    ...
    try:
        ...
        for event in events:
            assert (
                len(event) == 9
            )  # id, date, time, title, room, group_name, teacher_id, lesson_type, created_at
...
def test_get_events_for_date_function(simple_db):
    ...
    try:
        ...
        assert all(
            event[1] == "2025-09-16" for event in events
        )  # Всі події мають потрібну дату

        # Перевіряємо, що подія з іншої дати не повертається
        event_dates = [event[1] for event in events]
        assert (
            "2025-09-17" not in event_dates
        )  # Подія з 17 вересня не має бути в результаті
...
def test_add_subject_function(simple_db):
    ...
    try:
        ...
        subject_id = models.add_subject(
            name="Фізика", short_name="Фіз", credits=5, description="Загальна фізика"
        )
